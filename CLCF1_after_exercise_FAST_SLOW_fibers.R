...
# Download GDS file
gse28392 <- getGEO('GSE28392', GSEMatrix=TRUE)
gse28392 <- gse28392[[1]]

#########################################################################
# To get phenotype data (Samples)
p_data <- pData(gse28392)
p_data <- rownames_to_column(p_data, "Sample") %>% 
  dplyr::select(Sample, Age=characteristics_ch1.2, Type=characteristics_ch1.6, Trained=characteristics_ch1.4, TimePoint=characteristics_ch1.5) 
p_data$Age <- gsub("age: ","",p_data$Age)
p_data$Type <- gsub("fiber type: ","",p_data$Type)
p_data$Type <- gsub("-twitch","",p_data$Type)
p_data$Type <- gsub("-twitch","",p_data$Type)
...
p_data %<>% mutate(Age = gsub("\\s*\\([^\\)]+\\)", "", Age), Type = gsub("\\s*\\([^\\)]+\\)", "", Type), Trained = gsub("\\s*\\([^\\)]+\\)", "", Trained))
p_data %<>% mutate(group = paste(Age,Type,Trained,sep="_"))

p_data %>% count(Age, Type, Trained, TimePoint) %>% mutate(grp = paste(Trained,TimePoint,sep="_")) %>%
  ggplot(aes(x=Age, y=n, fill=grp)) + theme_bw() +
  geom_bar(stat = "identity") +
  geom_text(aes(label = n), position = position_stack(vjust=0.5), size=7) +
  facet_wrap(~Type) + labs(x="", y="", title = "Number of Samples") +
...
exprs_data <- exprs(gse28392)
feature_data <- fData(gse28392)
...
pca_data <- p_data %>% mutate(group = paste(Age, Type, Trained, sep="_")) %>% 
...
clcf1 <- merge(t(clcf1), p_data, by.x=0, by.y="Sample") %>% mutate(group2 = paste(Age,Trained, sep="_"))
head(clcf1)

#stat.test <- compare_means(CLCF1 ~ group, data=clcf1, method = "wilcox")
clcf1$group <- factor(clcf1$group, levels = unique(clcf1$group)[c(1,2,5,6,3,4,7,8)])
...
ggplot(clcf1, aes(x=as.factor(group), y=CLCF1, fill=TimePoint)) + theme_bw() +
  geom_point(position = position_jitterdodge( jitter.width=0.1, dodge.width = 0.75)) +
  geom_boxplot(outlier.shape = NA, alpha=0.5) + xlab("") +
  stat_summary(position = position_dodge(width = 0.75), geom="point", color="red", pch=18, size=3) +
  theme(axis.text.x=element_text(angle = 90, vjust = 0.5, hjust=1, size=12),
        panel.grid.minor.x = element_blank()) +
  stat_compare_means(label = "p.signif", method = "wilcox", label.y.npc=0.85, color="blue", size=15, hide.ns = TRUE) +
